...
def set_default_headers_on_response(response: Response) -> Response:
    ...
    """
    # Set CORS headers
    response.headers["Access-Control-Allow-Origin"] = "*" # solves POST request issue in frontend
    #
    # TODO: once app more mature and login capability in place, may need to modify setting bellow accordingly for better security
    #
    response.headers["Access-Control-Allow-Methods"] = "*" # solves PUT request issue from frontend
    response.headers["Access-Control-Allow-Headers"] = "Content-Type, Authorization" # solves POST request issue from frontend
    return response
